            ...
            try:
                ...
                if order:
                    ...
                    if order_detail and order_detail.get('state') == 'done':
                        # ✅ 업비트 API 응답에서 실제 값 가져오기
                        executed_volume = float(order_detail.get('executed_volume', 0))
                        paid_fee = float(order_detail.get('paid_fee', 0))
                        
                        # 실제 받은 금액 계산
                        # trades 배열에서 실제 체결 정보 가져오기
                        trades = order_detail.get('trades', [])
                        if trades:
                            total_received = sum(float(t['price']) * float(t['volume']) for t in trades)
                            total_fee = sum(float(t['fee']) for t in trades)
                            
                            actual_price = total_received / executed_volume if executed_volume > 0 else current_price
                            actual_quantity = executed_volume
                            
                            logger.info(f"체결 상세: 받은금액={total_received:,.0f}, 수수료={total_fee:,.0f}")
                        else:
                            # trades 정보가 없으면 기본 계산
                            actual_price = current_price
                            actual_quantity = quantity
                    else:
                        # 주문 상세 정보를 못 받은 경우
                        logger.warning(f"주문 상세 정보 조회 실패, 예상값 사용")
                        actual_price = current_price
                        actual_quantity = quantity
                    ...
                    # 매수 시 실제 지불 금액 (수수료 포함)
                    buy_cost = entry_price * entry_quantity
                    
                    # 매도 시 실제 받은 금액 (수수료 차감 후)
                    sell_revenue = actual_price * actual_quantity - paid_fee
                    
                    # 실제 PnL
                    real_pnl = sell_revenue - buy_cost
                    pnl_rate = (real_pnl / buy_cost) if buy_cost > 0 else 0.0
                    
                    logger.info(f"PnL 계산: 매수원가={buy_cost:,.0f}, "
                            f"매도수익={sell_revenue:,.0f}, "
                            f"순손익={real_pnl:+,.0f}")
                    ...
                    logger.info(f"🔴 매도 완료: {symbol} @ {actual_price:,.0f} KRW "
                                f"(PnL {real_pnl:+,.0f}, {pnl_rate:+.2%})")
                    return True
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # ✅ 2번: Adaptive Preset Manager - 자동 프리셋 조정
                if self.preset_manager:
                    ...
                    if current_time - self.last_preset_check >= preset_check_interval:
                        ...
                        try:
                            ...
                            if consecutive.get('type') == 'loss' and consecutive.get('count', 0) >= 4:
                                # 4회 이상 연속 손실 시 즉시 보수적 모드로
                                logger.warning(f"⚠️ 연속 {consecutive['count']}회 손실 감지 - 즉시 보수적 모드로 전환")
