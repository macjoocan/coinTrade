        ...
        if trade_type == 'buy':
            ...
            try:
                ...
                if order:
                    ...
                    logger.info(f"진입 수량: {entry_quantity:.8f}")
                    logger.info(f"매수 원가: {buy_cost:,.2f} KRW")
                    logger.info(f"")
                    logger.info(f"매도가: {actual_price:,.2f} KRW")
                    logger.info(f"매도 수량: {actual_quantity:.8f}")
        ...
        while True:
            try:
                ...
                if self.preset_manager and ADAPTIVE_PRESET_CONFIG['enabled']:
                    ...
                    check_interval = ADAPTIVE_PRESET_CONFIG.get('check_interval', 3600)
                    
                    if current_time - self.last_preset_check >= check_interval:
                        logger.info("🔍 자동 프리셋 조정 체크...")
                        self.preset_manager.auto_adjust_preset(TRADING_PAIRS)
