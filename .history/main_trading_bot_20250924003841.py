        ...
        try:
            ...
            for symbol in TRADING_PAIRS:
                ...
                if indicators:
                    ...
                    # 기존 포지션이 있다고 가정하고 매도 신호 체크
                    if symbol in bot.risk_manager.positions:
                        position = bot.risk_manager.positions[symbol]
                        current_price = indicators['price']
                        pnl_rate = (current_price - position['entry_price']) / position['entry_price']
                        
                        if pnl_rate >= bot.strategy.min_profit_target:
                            print(f"🔴 {symbol} 매도 신호! (목표 달성: {pnl_rate:.1%})")
