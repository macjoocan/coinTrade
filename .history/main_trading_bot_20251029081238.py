...
class TradingBot:
    ...
    def execute_averaging_down(self, symbol, current_price):
            ...
            try:
                ...
                MIN_ORDER_AMOUNT = 5500  # 5,500원 (수수료 고려)
    ...
    def is_averaging_completed(self, symbol):  # ✅ 여기부터 추가!
        """물타기가 완료되었는지 체크"""
        
        if not AVERAGING_DOWN_CONFIG['enabled']:
            return True  # 물타기 비활성화면 항상 완료로 간주
    ...
    def check_exit_conditions(self):
            ...
            MIN_ORDER_VALUE = 8000  # ✅ 업비트 최소 주문 금액
                ...
                try:
                    ...
                    if self.risk_manager.check_trailing_stop(symbol, current_price):
                        ...
                        if self.is_averaging_completed(symbol):
                            ...
                            # 물타기 진행 중 - 추적 손절 보류
                            avg_info = self.averaging_manager.get_averaging_info(symbol)
                            logger.info(f"{symbol}: 추적 손절 감지 - 물타기 우선 ({avg_info['count']}/{AVERAGING_DOWN_CONFIG['max_averaging_count']}차)")
                            # 추적 손절 무시하고 계속 진행
