...
class ImprovedStrategy:
    ...
    def calculate_entry_score(self, indicators):
        ...
        # 상승 지속 (중간 점수) ← 이게 중요!
        elif 50 < rsi < 70:  # 변경: 60 → 70
            if rsi < 60:
                score += 2.5  # 변경: 1 → 2.5
                details.append(f"RSI 상승 지속 ({rsi:.1f}) (+2.5)")
            else:
                score += 2.0  # 60-70 범위도 점수 줌
                details.append(f"RSI 강세 ({rsi:.1f}) (+2.0)")
        
        # 중립 구간
        elif 40 < rsi < 50:
            score += 1.5  # 변경: 2 → 1.5
            details.append(f"RSI 적정 ({rsi:.1f}) (+1.5)")
        
        # 과매수 경고
        elif rsi >= 70:
            score += 0.5
            details.append(f"RSI 과매수 주의 ({rsi:.1f}) (+0.5)")
