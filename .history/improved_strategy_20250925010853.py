...
class ImprovedStrategy:
    ...
    def should_enter_position(self, symbol, indicators):
        ...
        # 2. 쿨다운 체크 (선택적)
        if self.is_in_cooldown(symbol) and not indicators.get('strong_signal', False):
            return False, "쿨다운 중"
        ...
        # 강한 신호 (긴급 매수)
        if score >= 8:
            return True, f"강한 매수 신호! (점수: {score:.1f})"
        
        # 일반 신호
        elif score >= ADVANCED_CONFIG.get('entry_score_threshold', 5):
            return True, f"진입 조건 충족 (점수: {score:.1f})"
        
        # 소액 포지션 (테스트)
        elif score >= ADVANCED_CONFIG.get('min_score_for_small_position', 4):
            if indicators.get('rsi', 50) < 40:  # RSI가 낮을 때만
                return True, f"소액 진입 가능 (점수: {score:.1f}, RSI: {indicators['rsi']:.1f})"
        
        return False, f"진입 조건 미충족 (점수: {score:.1f}/5)"
