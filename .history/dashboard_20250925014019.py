...
class TradingDashboard:
    ...
    def get_price_table(self):
        ...
        for symbol in TRADING_PAIRS:
            ticker = f"KRW-{symbol}"
            try:
                # get_ticker 대신 개별 함수 사용
                current_price = pyupbit.get_current_price(ticker)
                
                # get_ticker는 딕셔너리나 리스트를 반환할 수 있음
                ...
                if current_price:
                    ...
                    if isinstance(ticker_info, dict):
                        # 딕셔너리인 경우
                        change_rate = ticker_info.get('signed_change_rate', 0) * 100
                        volume = ticker_info.get('acc_trade_volume_24h', 0)
                    elif isinstance(ticker_info, list) and len(ticker_info) > 0:
                        # 리스트인 경우 (기존 코드)
                        change_rate = ticker_info[0].get('signed_change_rate', 0) * 100
                        volume = ticker_info[0].get('acc_trade_volume_24h', 0)
                    else:
                        # ticker_info가 없는 경우
                        change_rate = 0
                        volume = 0
                ...
                # 에러 발생 시 에러 메시지 표시
                console.print(f"[red]Error for {symbol}: {str(e)[:50]}[/red]")
