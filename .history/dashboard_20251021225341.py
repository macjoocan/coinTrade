...
class TradingDashboard:
    ...
    def get_recent_trades_panel(self):
        ...
        if not trades:
            return Panel("[dim]거래 내역 없음[/dim]", title="📜 Recent Trades", border_style="blue")
        ...
        for trade in trades:
            ...
            table.add_row(
                ...
                f"[{pnl_color}]{trade['pnl_rate']:+.1%}[/{pnl_color}]"
            )
    ...
    def get_stats_panel(self, days, title):
        ...
        table.add_row("Gross PnL", f"[dim]{stats['total_pnl']:+,.0f}[/dim]")
        ...
        if stats['trade_count'] > 0:
            ...
            # 평균 보유 시간
            table.add_row("Avg Hold", f"{stats['avg_hold_time']:.1f}h")
            ...
            # 최고/최악
            if stats['max_win'] > 0:
                table.add_row("Max Win", f"[dim]+{stats['max_win']:,.0f}[/dim]")
            if stats['max_loss'] < 0:
                table.add_row("Max Loss", f"[dim]{stats['max_loss']:,.0f}[/dim]")
