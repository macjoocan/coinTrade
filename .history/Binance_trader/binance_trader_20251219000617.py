...
class Config:
    ...
    # ✅ 누락되었던 시뮬레이션 초기 자금 설정 추가
    SIMULATION_BALANCE = 10000  # 기본값 10,000 USDT
...
class Strategy:
    ...
    def bollinger(df: pd.DataFrame) -> Optional[str]:
        ...
        if df['close'].iloc[-1] <= bb.bollinger_lband().iloc[-1]: return 'BUY'
        if df['close'].iloc[-1] >= bb.bollinger_hband().iloc[-1]: return 'SELL'
        return None
    ...
    @classmethod
    def analyze(cls, df_base: pd.DataFrame, df_trend: pd.DataFrame) -> Dict[str, any]:
        """✅ 타점(1h) 신호와 추세(4h)를 결합하여 분석"""
        # 1시간 봉 지표 신호 분석
        indicators = {
            'RSI': cls.rsi(df_base),
            'MA_Cross': cls.ma_cross(df_base),
            'MACD': cls.macd(df_base),
            'Bollinger': cls.bollinger(df_base),
            'Stochastic': cls.stochastic(df_base)
        }
        
        # 4시간 봉 추세 분석
        market_trend = cls.get_trend(df_trend)
        
        buy_count = sum(1 for s in indicators.values() if s == 'BUY')
        sell_count = sum(1 for s in indicators.values() if s == 'SELL')
        
        # 최종 신호 결정 (추세 일치 여부 확인)
        final_signal = None
        if buy_count >= Config.STRATEGY_THRESHOLD and market_trend == 'UP':
            final_signal = 'LONG'
        elif sell_count >= Config.STRATEGY_THRESHOLD and market_trend == 'DOWN':
            final_signal = 'SHORT'
...
class TradingBot:
    ...
    def scan(self):
        for symbol in Config.SYMBOLS:
            # 1. 1시간 봉(타점) 및 4시간 봉(추세) 데이터 모두 가져오기
            ...
            # 2. 통합 분석 실행
            analysis = Strategy.analyze(df_1h, df_4h)
            decision = analysis['final_decision']
            
            # 3. 신호에 따른 포지션 오픈
            if decision == 'LONG' and symbol not in self.positions.positions:
                self.positions.open(symbol, 'BUY')
            elif decision == 'SHORT' and symbol not in self.positions.positions:
