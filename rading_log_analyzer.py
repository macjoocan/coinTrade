...
class TradingLogAnalyzer:
    ...
    def parse_log(self):
        ...
        try:
            ...
            print(f"❌ {self.log_file} 파일을 찾을 수 없습니다")
            return []
        ...
        for line in lines:
            try:
                ...
                if '매수 완료' in line or '✅ 매수 완료' in line:
                    ...
                    if match and symbol_match and price_match:
                ...
                # 매도 기록
                    ...
                    if not all([match, symbol_match, price_match]):
                        skipped_lines += 1
                        continue
                    ...
                    # PnL이 없으면 직접 계산
                    if pnl == 0.0 and current_trade.get('entry_price'):
                        pnl_rate = (exit_price - current_trade['entry_price']) / current_trade['entry_price']
                    ...
                    if current_trade.get('symbol') == symbol:
                        ...
                        # 완전한 거래만 추가
                        if all(k in current_trade for k in ['entry_price', 'exit_price', 'pnl_rate']):
        ...
        print(f"   파싱된 라인: {parsed_lines}개")
        if skipped_lines > 0:
            print(f"   스킵된 라인: {skipped_lines}개")
        
        if len(self.trades) == 0:
            print("\n⚠️  파싱된 거래가 없습니다. 로그 형식 샘플:")
            print("-"*80)
            with open(self.log_file, 'r', encoding='utf-8') as f:
