...
class AdaptivePresetManager:
    ...
    def __init__(self, config):
        ...
        # config에서 전환 간격 로드 (기본 2시간)
        self.min_switch_interval = config.get('min_switch_interval', 3600 * 2)  # 최소 2시간 간격
        ...
        # 임계값 설정 (더 민감하게 조정)
        self.thresholds = {
            'high_volatility': 0.035,     # 3.5% 이상 (더 민감)
            'medium_volatility': 0.02,    # 2-3.5%
            'low_volatility': 0.02,       # 2% 이하

            'high_win_rate': 0.60,        # 60% 이상 (완화)
            'medium_win_rate': 0.45,      # 45-60%
            'low_win_rate': 0.45,         # 45% 이하

            'consecutive_losses': 2,      # 연속 손실 (3 → 2)
            'consecutive_wins': 2,        # 연속 수익 (3 → 2)
        }
