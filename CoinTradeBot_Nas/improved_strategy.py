...
try:
    ...
    ADVANCED_ML_CONFIG = {'enabled': False}
...
class ImprovedStrategy:
    def __init__(self):
        ...
        # 🆕 실시간 시장 감지기
        self.rapid_detector = get_rapid_detector() if RAPID_MARKET_CONFIG.get('enabled', True) else None
        ...
        # 🆕 고급 ML 엔진 또는 기본 ML 사용
        self.use_advanced_ml = ADVANCED_ML_CONFIG.get('enabled', False) and ADVANCED_ML_AVAILABLE

        if self.use_advanced_ml:
            logger.info("🚀 고급 ML 엔진 (앙상블 + LSTM) 활성화")
            self.advanced_ml = get_advanced_ml_engine()
            self.ml_generator = None  # 기본 ML 비활성화

            if not self.advanced_ml.is_trained:
                logger.info("🤖 고급 ML 모델 초기 학습을 시작합니다...")
                self.advanced_ml.train(TRADING_PAIRS)

            self.ml_min_probability = ADVANCED_ML_CONFIG['prediction']['min_probability']
            self.ml_min_confidence = ADVANCED_ML_CONFIG['prediction']['high_confidence']

        elif ML_CONFIG['enabled']:
            logger.info("📊 기본 ML 엔진 (RandomForest) 활성화")
    ...
    def should_enter_position(self, symbol, indicators):
        ...
        # 5-3. 머신러닝 예측 (고급 ML 또는 기본 ML)
        if self.use_advanced_ml and self.advanced_ml:
            # 🆕 고급 ML 엔진 사용 (앙상블 + LSTM)
            try:
                ml_prediction = self.advanced_ml.predict(symbol)
                if ml_prediction:
                    signal_scores['ml'] = ml_prediction['probability']
                    signal_details['ml'] = [
                        f"🚀 고급ML 확률: {ml_prediction['probability']:.1%}",
                        f"앙상블: {ml_prediction['ensemble_prob']:.1%}",
                        f"LSTM: {ml_prediction['lstm_prob']:.1%}"
                    ]
                else:
                    signal_scores['ml'] = 0.5
                    signal_details['ml'] = ["고급ML 예측 불가"]
            except Exception as e:
                logger.warning(f"고급 ML 예측 실패: {e}")
                signal_scores['ml'] = 0.5
                signal_details['ml'] = ["고급ML 오류"]

        elif ML_CONFIG['enabled'] and self.ml_generator:
            # 기본 ML 엔진 사용
            try:
        ...
        # 7. 🆕 실시간 시장 분석 (rapid_market_detector 사용)
        if self.rapid_detector and RAPID_MARKET_CONFIG.get('enabled', True):
            # 실시간 시장 분석
            market_state = self.rapid_detector.analyze(TRADING_PAIRS)
            market_condition = market_state['condition']
            rapid_adjustment = market_state['score_adjustment']

            # 진입 안전 체크
            is_safe, safety_msg = self.rapid_detector.is_safe_to_enter()
            if not is_safe:
                logger.warning(f"🚫 {symbol}: {safety_msg}")

            adjustment = rapid_adjustment
            adjusted_threshold = base_threshold + adjustment

            logger.info(f"🚀 실시간 시장: {market_condition.upper()}, "
                        f"변동성: {market_state['volatility']}, "
                        f"모멘텀: {market_state['momentum']}")
            logger.info(f"   기준: {base_threshold:.1f} → {adjusted_threshold:.1f} "
                        f"(조정: {adjustment:+.1f})")
