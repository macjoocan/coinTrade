...
class AdaptiveScoreManager:
    ...
    def __init__(self,
        ...
        # 점수 조정 설정 (더 적극적인 기본값)
        self.config = {
            'min_trades_for_analysis': 5,       # 분석에 필요한 최소 거래 수 (5건으로 축소)
            'lookback_days': 7,                  # 분석할 기간 (일)
            'target_win_rate': 0.50,             # 목표 승률
            'min_win_rate': 0.40,                # 최소 허용 승률
            'max_adjustment': 0.8,               # 최대 조정폭 (0.5 → 0.8)
            'adjustment_step': 0.15,             # 조정 단위 (0.1 → 0.15)
            'score_min': 4.0,                    # 최소 진입 점수
            'score_max': 8.0,                    # 최대 진입 점수
            'consecutive_loss_threshold': 2,    # 연속 손실 임계값 (3 → 2)
            'hot_streak_threshold': 2,          # 연속 수익 임계값 (3 → 2)
        }
