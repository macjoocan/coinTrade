...
from rapid_market_detector import get_rapid_detector  # 🆕 실시간 시장 감지
from market_condition_check import MarketAnalyzer  # 🆕 공유 시장 분석기
from adaptive_risk_manager import get_adaptive_risk_manager, get_regime_detector  # 🆕 ATR 동적 리스크 + 마켓 레짐
...
from config import (
    ...
    RAPID_MARKET_CONFIG,  # 🆕 실시간 시장 감지 설정
    ADAPTIVE_RISK_CONFIG,  # 🆕 ATR 동적 리스크 설정
    MARKET_REGIME_CONFIG,  # 🆕 마켓 레짐 설정
    ADVANCED_ML_CONFIG  # 🆕 고급 ML 설정 (주기적 재학습)
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.getenv('DATA_PATH', ''), 'trading.log') if os.getenv('DATA_PATH') else 'trading.log', encoding='utf-8'),
...
class TradingBot:
    def __init__(self, access_key, secret_key):
        ...
        # 🆕 실시간 시장 감지 시스템 초기화
        if RAPID_MARKET_CONFIG.get('enabled', True):
            self.rapid_detector = get_rapid_detector()
            logger.info("🚀 실시간 시장 감지 시스템 활성화")
            logger.info(f"   캐시 주기: {RAPID_MARKET_CONFIG.get('cache_duration', 30)}초")
            logger.info(f"   급변 임계값: {RAPID_MARKET_CONFIG.get('rapid_change_threshold', 0.02):.1%}")
        else:
            self.rapid_detector = None
            logger.info("🚀 실시간 시장 감지 시스템 비활성화")

        # 현재 시장 상황 캐시 (익절 조정용)
        self.current_market_condition = 'neutral'
        self.last_market_check_for_tp = 0

        # 🆕 공유 MarketAnalyzer (중복 API 호출 방지)
        self.market_analyzer = MarketAnalyzer()
        logger.info("📊 공유 MarketAnalyzer 초기화 완료")

        # 🆕 ATR 기반 동적 리스크 관리자 초기화
        if ADAPTIVE_RISK_CONFIG.get('enabled', True):
            self.adaptive_risk_manager = get_adaptive_risk_manager(ADAPTIVE_RISK_CONFIG)
            logger.info("📊 ATR 동적 리스크 관리자 활성화")
            logger.info(f"   손절 배수: ATR x {ADAPTIVE_RISK_CONFIG.get('atr_multiplier_stop', 1.5)}")
            logger.info(f"   익절 배수: ATR x {ADAPTIVE_RISK_CONFIG.get('atr_multiplier_take', 2.5)}")
        else:
            self.adaptive_risk_manager = None
            logger.info("📊 ATR 동적 리스크 관리자 비활성화")

        # 🆕 마켓 레짐 감지 시스템 초기화
        if MARKET_REGIME_CONFIG.get('enabled', True):
            self.regime_detector = get_regime_detector(MARKET_REGIME_CONFIG)
            self.current_regime = None
            self.last_regime_check = 0
            logger.info("🔄 마켓 레짐 감지 시스템 활성화")
            logger.info(f"   변동성 관찰: {MARKET_REGIME_CONFIG.get('volatility_lookback', 24)}시간")
            logger.info(f"   추세 관찰: {MARKET_REGIME_CONFIG.get('trend_lookback', 48)}시간")
        else:
            self.regime_detector = None
            self.current_regime = None
            logger.info("🔄 마켓 레짐 감지 시스템 비활성화")

        # 🆕 ML 주기적 재학습 체크 (12시간마다 체크)
        self.last_ml_retrain_check = time.time()
        self.ml_retrain_check_interval = 12 * 3600  # 12시간마다 체크
        ml_retrain_days = ADVANCED_ML_CONFIG.get('training', {}).get('auto_retrain_days', 3)
        logger.info(f"🤖 ML 자동 재학습 시스템 초기화 (재학습 주기: {ml_retrain_days}일)")
    ...
    def _check_and_retrain_ml(self):
        """🆕 ML 모델 재학습 필요 여부 체크 및 실행"""
        try:
            # strategy에서 advanced_ml 가져오기
            if not hasattr(self.strategy, 'advanced_ml') or self.strategy.advanced_ml is None:
                return

            ml_engine = self.strategy.advanced_ml
            needs, reason = ml_engine.needs_retraining()

            if needs:
                logger.info("")
                logger.info("=" * 60)
                logger.info("🤖 ML 자동 재학습 시작")
                logger.info(f"   사유: {reason}")
                logger.info("=" * 60)

                # 백그라운드에서 재학습 (포지션이 없을 때만)
                if len(self.risk_manager.positions) == 0:
                    result = ml_engine.auto_retrain_if_needed(TRADING_PAIRS)
                    if result.get('retrained'):
                        logger.info("✅ ML 모델 재학습 완료!")
                        if 'ensemble' in result:
                            acc = result['ensemble'].get('accuracy', 0)
                            logger.info(f"   앙상블 정확도: {acc:.1%}")
                    else:
                        logger.info(f"   재학습 스킵: {result.get('reason', '알 수 없음')}")
                else:
                    logger.info("   ⏳ 보유 포지션 있음 - 다음 체크 시 재학습 예정")

                logger.info("=" * 60)
                logger.info("")
            else:
                # 디버그용 (필요시 활성화)
                # logger.debug(f"ML 재학습 체크: {reason}")
                pass

        except Exception as e:
            logger.error(f"ML 재학습 체크 중 오류: {e}")
    ...
    def get_current_regime(self):
        ...
        cache_duration = MARKET_REGIME_CONFIG.get('cache_duration', 300)

        # 캐시된 레짐 사용 (5분)
        if self.current_regime and (current_time - self.last_regime_check) < cache_duration:
            return self.current_regime

        try:
            self.current_regime = self.regime_detector.detect_regime(TRADING_PAIRS)
    ...
    def get_dynamic_risk_params(self, symbol):
        """🆕 ATR + 레짐 기반 동적 리스크 파라미터 반환"""
        if not self.adaptive_risk_manager:
            return {
                'stop_loss': RISK_CONFIG['stop_loss'],
                'take_profit': STRATEGY_CONFIG['min_profit_target'],
        ...
        # ATR 기반 동적 파라미터 계산
        params = self.adaptive_risk_manager.get_dynamic_risk_params(symbol, market_regime=regime)

        return params

    def get_adaptive_take_profit_target(self, symbol=None):
        """🆕 ATR + 레짐 + 시장상황 기반 동적 익절 목표 반환"""
        base_target = STRATEGY_CONFIG['min_profit_target']

        # 1. ATR 기반 동적 익절 (우선 적용)
        if self.adaptive_risk_manager and symbol:
            try:
                regime = self.get_current_regime()
                atr_target = self.adaptive_risk_manager.get_dynamic_take_profit(
                    symbol, base_target, market_regime=regime
                )

                # 레짐에 따른 추가 조정
                if regime:
                    adjustments = self.regime_detector.get_strategy_adjustments(regime)
                    atr_target *= adjustments.get('take_profit_mult', 1.0)

                # 최소/최대 범위 적용
                atr_target = max(
                    ADAPTIVE_TAKE_PROFIT_CONFIG['min_profit_floor'],
                    min(atr_target, ADAPTIVE_RISK_CONFIG.get('max_take_profit', 0.08))
                )

                return atr_target

            except Exception as e:
                logger.debug(f"ATR 기반 익절 계산 실패: {e}")
        ...
        if current_time - self.last_market_check_for_tp > 60:
            try:
                # ✅ 공유 MarketAnalyzer 사용 (중복 API 호출 방지)
    ...
    def execute_trade(self, symbol, trade_type, current_price=None, force_stop_loss=False):
        ...
        if trade_type == 'buy':
            ...
            # 🆕 실시간 시장 상태에 따른 포지션 크기 조정
            if self.rapid_detector and RAPID_MARKET_CONFIG.get('enabled', True):
                size_multiplier = self.rapid_detector.get_position_size_multiplier()
                if size_multiplier != 1.0:
                    original_quantity = quantity
                    quantity = quantity * size_multiplier
                    logger.info(f"🚀 시장 상태 포지션 조정: {original_quantity:.8f} → {quantity:.8f} (x{size_multiplier})")
            ...
            for symbol in symbols:
                try:
                    ...
                    if partial_exit:
                        ...
                        # ✅ 버그 수정: 전역 stop_loss 대신 포지션별 breakeven_mode 사용
                        self.risk_manager.positions[symbol]['breakeven_mode'] = True
                        logger.info(f"✅ {symbol} 1차 분할 익절 완료. 남은 물량 본절 방어 모드 진입")
                    
                    # 2. ✅ ATR + 레짐 기반 동적 손절 체크 (보유시간 무시)
                    current_regime = self.get_current_regime() if hasattr(self, 'regime_detector') else None
                    if self.risk_manager.check_stop_loss(
                        symbol, current_price, self.averaging_manager,
                        self.volatility_monitor,  # 변동성 모니터 (폴백)
                        getattr(self, 'adaptive_risk_manager', None),  # 🆕 ATR 동적 리스크
                        current_regime  # 🆕 마켓 레짐
                    ):
                    ...
                    # 4. 목표 수익 체크 (남은 수량 전량 매도) + 🆕 ATR + 레짐 기반 동적 익절
                    current_pnl_rate = (current_price - entry_price) / entry_price

                    # 🆕 ATR + 레짐 + 시장상황 기반 동적 익절 목표 가져오기
                    adaptive_target = self.get_adaptive_take_profit_target(symbol)
    ...
    def print_status(self):
        ...
        # 시장 상황 표시 (공유 MarketAnalyzer 사용)
        market = self.market_analyzer.analyze_market(TRADING_PAIRS)
    ...
    def run(self):
        ...
        preset_check_interval = ADAPTIVE_PRESET_CONFIG.get('check_interval', 3600)  # 1시간
        ...
        while True:
            try:
                ...
                # 🆕 실시간 시장 감지 - 급락장 긴급 대응
                if self.rapid_detector and RAPID_MARKET_CONFIG.get('enabled', True):
                    market_state = self.rapid_detector.analyze(TRADING_PAIRS)

                    # 급락장 감지 시 긴급 청산 체크
                    should_close, close_reason = self.rapid_detector.should_close_positions()
                    if should_close and len(self.risk_manager.positions) > 0:
                        logger.warning("")
                        logger.warning("=" * 60)
                        logger.warning(f"🚨 긴급 상황 감지: {close_reason}")
                        logger.warning(f"   시장 상태: {market_state['condition'].upper()}")
                        logger.warning(f"   변동성: {market_state['volatility']}")
                        logger.warning(f"   모멘텀: {market_state['momentum']}")
                        logger.warning("=" * 60)

                        # 보유 포지션 긴급 청산
                        self.force_close_all_positions("급락장 긴급 대응")
                ...
                if self.preset_manager and ADAPTIVE_PRESET_CONFIG['enabled']:
                    ...
                    # 주기적 체크 (기본 1시간)
                    if current_ts - self.last_preset_check >= preset_check_interval:
                        logger.info("\n" + "="*60)
                        ...
                        try:
                            ...
                            self.last_preset_check = current_ts
                ...
                # 🆕 ML 주기적 재학습 체크 (12시간마다)
                if time.time() - self.last_ml_retrain_check >= self.ml_retrain_check_interval:
                    self.last_ml_retrain_check = time.time()
                    self._check_and_retrain_ml()
